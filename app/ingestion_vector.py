...
from langchain_text_splitters import RecursiveCharacterTextSplitter, HTMLTextSplitter
...
from app.config import CHROMA_STORE_DIR, DATA_DIR, METADATA_FILE, WEB_META_FILE, PROJECT_ROOT, CHUNK_SIZE, CHUNK_OVERLAP
...
class DocumentProcessor:
    ...
    def _create_html_text_splitter(self, chunk_size: Optional[int] = CHUNK_SIZE,
                              chunk_overlap: Optional[int] = CHUNK_OVERLAP,
                              separators: List[str] = None) -> RecursiveCharacterTextSplitter:

        print(f"chunk_size type: {type(chunk_size)}, value: {chunk_size}")
        print(f"chunk_overlap type: {type(chunk_overlap)}, value: {chunk_overlap}")
        """Create a text splitter with specified parameters."""
        if separators is None:
            separators = ["\n\n", "\n", ".", "!", "?", ",", " ", ""]

        return HTMLeCharacterTextSplitter(
    ...
    def _create_text_splitter(self, chunk_size: Optional[int] = CHUNK_SIZE,
                              chunk_overlap: Optional[int] = CHUNK_OVERLAP,
                              separators: List[str] = None) -> RecursiveCharacterTextSplitter:
    ...
    def ingest_web_pages(self, chunk_size: Optional[int] = CHUNK_SIZE,
                         chunk_overlap: Optional[int] = CHUNK_OVERLAP) -> List[Document]:
        ...
        try:
            for item in self.website_data.get("MoneyMarket", []):
                ...
                try:
                    ...
                    if _documents:
                        text_splitter = self._create_html_text_splitter(
                            chunk_size=chunk_size if chunk_size else self.default_chunk_size,
                            chunk_overlap=chunk_overlap if chunk_overlap else self.default_chunk_overlap
                        )
    ...
    def ingest_pdf_documents(self, directory: str = None,
                             chunk_size: Optional[int] = CHUNK_SIZE,
                             chunk_overlap: Optional[int] = CHUNK_OVERLAP) -> List[Document]:
        ...
        try:
            for filename in os.listdir(directory):
                if filename.lower().endswith(".pdf"):
                    file_path = os.path.join(directory, filename)
                    try:
                        documents = self._load_documents(file_path, PyPDFLoader)
                        chunks = self._split_and_process_documents(
                            documents,
                            file_path,
                            chunk_size,
    ...
    def calculate_document_id(self, chunks: List[Document]) -> List[Document]:
        ...
        try:
            ...
            stack_trace = traceback.format_exc()
            error_msg = f"Error calculating IDs for chunks: {e}"
            logger.error(error_msg)
            logging.error(f"Stack trace:\n{stack_trace}")
            raise DocumentProcessingException(error_msg) from e
    ...
    def ingest_all(self) -> List[Document]:
        ...
        try:
            documents = self.ingest_pdf_documents()
    ...
    try:
        ...
        if all_documents:
            try:
                ...
                # Test query
                query = "How to maximizing contributions to 401k"
                ...
                for i, (doc, score) in enumerate(results):
                    ...
                    logger.info(f" Source: {doc.metadata['source']} Content: {doc.page_content[:1000]}...")
